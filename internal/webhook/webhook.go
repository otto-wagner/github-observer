...
func Create(configuration Config) {
	...
	for _, webhook := range configuration.Webhooks {
		for _, repo := range configuration.Repositories {
			...
			hookResponse, resp, err := client.Repositories.CreateHook(context.Background(), repository.Owner, repository.Name, &github.Hook{
				Config: &github.HookConfig{
					URL:         github.Ptr(webhook.PayloadUrl),
					ContentType: github.Ptr(webhook.ContentType),
					Secret:      github.Ptr(configuration.HmacSecret),
					InsecureSSL: github.Ptr(webhook.InsecureSsl),
				},
				Events: webhook.Events,
				Active: github.Ptr(true),
			})
			if resp.Response.StatusCode == http.StatusUnprocessableEntity {
